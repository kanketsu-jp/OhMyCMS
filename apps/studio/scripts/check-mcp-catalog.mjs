 ...
 *    （`https://…` は `:` の直後なので当たらない）。
 *    🟢 実測 2026-08-16: `"https://example.com/x"` は**行頭でも行の途中でも残る**。
 *
 * ## 🚨 この関数が**消しすぎる**形（実測 2026-08-16・**書いていなかったので足した**）
 * ```
 * 🚨 文字列の中の「空白 + //」 …… `const u = " //zz";` → `const u = " ` に**切れる**
 * 🚨 文字列の中のブロックコメント記号（スラッシュ+星 … 星+スラッシュ）… **丸ごと消える**
 * ```
 *    ＝ **文字列とコメントを区別していません**（引用符を一切見ていない作り）。
 *    そのぶん、polish が `strip-comments.mjs` で見つけた
 *    「**正規表現の中の引用符から先、コメントが落ちなくなる**」穴は**ありません**
 *    （🟢 実測: 二重引用符・単一引用符を含む正規表現のあとでも、コメントは潰れる）。
 *    ＝ 🚨 **消し漏らす側ではなく、消しすぎる側**に倒れています。
 *    登録を数える用途では**消しすぎ＝登録を見落とす**ので、
 *    「登録 0 件なら落とす」「目録と突き合わせる」で**別の側から捕まえています**。
 *
 * 🚨 **`strip-comments.mjs` を import していません**（自前）。
 *    ＝ **polish の 9 本の測定にも、toast の直しにも入りません。** 上の実測は私が別に取ったもの。
 *
