 ...
 *   - 正規表現リテラル（`/foo\/\/bar/` のような形）は文字列として扱っていない。
 *     🚨 **2026-08-16 に 4 通り測ったが、どれも誤認しなかった**（polish が共通の
 *     `strip-comments.mjs` で直した「正規表現の中に引用符が 1 つ在ると、そこから先の
 *     コメントを落とさなくなる」形も含む）:
 *       `/\/\/ x/` ／ `/\/\/ method: "POST"/` ／ テンプレートリテラルの `https://`
 *       ／ 文字列の `"https://example.com"` … **すべてコード扱い（正しい）**
 *       🟢 対照 ブロックコメントの中 … **コメント扱い（正しい）**
 *     🚨 **「誤認する可能性がある」と書いていたが、測ったら 4 通りとも起きなかった。**
 *     **死角を過大に書くのも、書かないのと同じくらい人を誤らせる**（要らない手当てをさせる）。
 *     **測った 4 通りの外は、まだ分からない**——そこは変わらない。
