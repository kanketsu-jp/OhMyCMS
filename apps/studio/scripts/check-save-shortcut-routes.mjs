 ...
 *    実測（shell・2026-08-16）: 宣言が消えた画面でも、**欄を埋めて ⌘Enter を押すと送信された**
 *      policies / roles / labels … submit **1** ／ agents … submit **0** ／ users … **測れず**
 *      🟢 陰性対照 ⌘B … submit 0（＝ その計器は 0 も 1 も出せる）
 *    ＝ **素のフォーム送信（焦点が欄の中のときの Enter の既定動作）が肩代わりしうる。**
 *      この検査には、それが**原理的に見えない**。
 *
 * 🚨 **したがって、この検査が赤くなっても「⌘Enter が効かない」とは言えない。**
 *    言えるのは「**宣言された鍵が消えた**」まで。両者の差は**焦点の位置**で出る:
 *      宣言された鍵 … `use-shortcut.ts` が **`document` に付く** → **焦点がどこでも効く**
 *      素の送信 …… **焦点が欄の中のときだけ**効く
 *    ＝ 利用者から見た差は「**入力し終えて画面のどこかをクリックしたあと、押しても保存されない**」。
 *    🚨 **その差はまだ測っていない**（焦点を `document.body` に置いて押す実験が要る）。
 *
