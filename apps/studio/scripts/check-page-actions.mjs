 ...
 *
 * 🚨 **▾（オプション）の中は、この検査から外れます**（2026-08-16・司令塔の決定②）。
 *
 * 283 A で「削除は主アクションから外し、▾ の中へ」が決まった。
 * 🚨 `PAGE_ACTIONS` は **kind: link / submit / button** と `role: primary|secondary` だけで、
 *    **▾ の中を宣言する欄がありません**。
 *    → **▾ へ移した操作は、`labelKey` / `form` / `href` / `role` の 4 つとも見なくなります。**
 *    → 🚨 **辞書キーの実在も、form の id も、行き先も見ません。**
 *    ＝ **いちばん危ない操作（削除）が、いちばん検査の外へ出ます。**
 *
 * 🚨 **なぜ、いま広げないか**（＝「除外は理由とセット」）:
 *    **▾ の部品（`components/admin/page-action.tsx` / base2）の形が、いま動いています。**
 *    動いている間に宣言の形を決めると、**固まったときにもう一度決め直す**ことになります。
 *
 * 🚨 **いつ広げるか**: base2 の ▾ の形が固まったとき。
 *    そのとき宣言に `kind:"menu"` のような欄を足せば、**この 4 つの検査が ▾ の中にも戻ります**。
 *    🚨 **決めたのは 2026-08-16。「移り終わってからだと『元から見ていなかった』と
 *      区別が付かない」ので、移す前に書いてあります。**
 *
