...
const TIPTAP_CONFLICT_EXCEPTIONS = {
  ...
  "mod+enter": {
    ...
    openQuestion:
      ...
      "同ファイルの原文は「`preventDefault` は伝播を止めないので、document で待っている" +
      "アプリのショートカットはそのまま動く（**実測で確かめる**）」で、**確かめた記録が無い**。" +
      "🚨 これは改行が混ざるより重い——**⌘Enter が黙って保存しなくなる**形。" +
      "測り方は 1 画面で足りる（item-form の本文にカーソルを置いて ⌘Enter → 保存されたか / 改行が入っていないか。" +
      "🟢 対照: 本文の外で ⌘Enter → 保存だけ）。**共有 DB に 1 行書くので、着手の可否は司令塔が決める**",
    reason:
