...
const TIPTAP_CONFLICT_EXCEPTIONS = {
  ...
  "mod+enter": {
    ...
    openQuestion:
      ...
      "🚨 **追記（同日・夕方）: この実測は、いまの木では再現しません。** 同じ URL を測り直したら "  +
      "**submit 0 件**で、画面には `contenteditable` が **0 個**・送信ボタンが **0 個**でした"  +
      "（287 の「表示と編集を分ける」が入り、**既定が表示モード**になったため。押す対象が画面に無い）。"  +
      "🚨 **消していないのは、時点が違うからです**——朝の実測は当時の木では正しく、"  +
      "いまの木では**確かめられていない**、が正確な状態。**「動く」と読まないこと。**"  +
      "測り直すなら、**先に「編集する」を押して編集モードにしてから**測ること。",
    reason:
