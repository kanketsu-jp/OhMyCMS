...
/**
 * 🚨 **行頭しか見ない**（引用符を一度も見ない）。2026-08-16 に polish の 2 つの見本で実測:
 * ```
 * 🟢 `const u = "https://example.com"; if (x === FOO) {}` … **FOO は残る**
 *    ＝ 文字列の中の `//` で行が消える「見逃し」は**起きない**
 * 🚨 `const x = 1; // FOO` ……………………………… **FOO を拾う**（**過検出**）
 *    ＝ **行の途中から始まるコメントは落とせない**
 * 🟢 対照 `const y = FOO;` … 残る ／ 🟢 対照 `// FOO` … 消える
 *    （🚨 **対照を 2 つ入れる**。「全部残る」でも「全部消える」でも見分けが付かないため）
 * ```
 * 🚨 過検出側に倒してある（取りこぼすと気づけないが、過検出は人が 1 件見に行くだけ）。
 * 🚨 共有の `strip-comments.mjs` へ寄せるときは、あちらが持っていた
 *    「**正規表現の中の引用符で、そこから先を落とさなくなる**」穴を持ち込まないこと
 *    （`8bfccf0` で直っているが、ここは**引用符を見ない**ので構造的に無縁）。
 */
