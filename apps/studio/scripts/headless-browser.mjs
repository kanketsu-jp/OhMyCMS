 ...
 *
 * 13. 🚨 **スクロールする領域を測るときは、スクロール位置も一緒に記録する。**
 *    2026-08-16（別のペインの実測）: 監査が指した「最後の行」は **DOM の最後の行ではなく、
 *    その位置で見えている最後の行**だった。**スクロール位置が変われば、切れる行も変わる**。
 *    ＝ **同じ画面を 2 回測ると、違う行が「最後」になる**（**位置を書いていない測定は再現しない**）。
 *    → `where()` が `scroll=<window の scrollY>` を毎回出すようにした。
 *      **1 行目に `where()` を出す**規律に乗るので、覚えていなくても記録される。
 *    🚨 **含まれないもの**: **内側のスクロール領域**（`overflow-y:auto` の箱）の `scrollTop`。
 *      そこを測る人は**自分で記録**してください（この関数は window しか見ていません）。
 *    🚨 そして **「最後の行」と書かない**。**「その位置で見えている最後の行」**と書く。
 *
...
class Session {
  ...
  async #whereImpl() {
    ...
    // 🚨 落とし穴13: **スクロール位置も記録する**。
    //    スクロールする領域を測ると、**同じ画面でも「見えている最後の行」が変わる**ので、
    //    位置を書いていない測定は**再現できない**（2026-08-16・別のペインの実測から）。
    //    🚨 ここで取るのは **window のスクロール量だけ**。
    //    **内側のスクロール領域（overflow-y:auto の箱）は含みません**——そこを測る人は、
    //    その箱の `scrollTop` を自分で記録してください。
    let scroll = "?";
    try {
      scroll = await this.eval(`String(Math.round(scrollY))`);
    } catch {
      // 同上。
    }
    return `url=${url} lang=${lang} status=${this.lastStatus ?? "?"} scroll=${scroll}`;
  }
