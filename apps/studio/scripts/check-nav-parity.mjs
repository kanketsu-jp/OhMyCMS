...
// 🚨 **この検査に「比率」「平均」の守りが無いのは、入れ忘れではありません**（2026-08-16）。
//    司令塔が配った 3 つの守り（走査÷候補 / 平均文字数 / 床）は、**複数ファイルを歩く検査**向け。
//    ここは **`loadSources()` が layout.tsx を 1 本だけ読む**ので:
//      ・比率 … **常に 1.0**（候補も走査も 1）＝ 動かないので守りにならない
//      ・平均 … **平均 = 合計**＝ 平均にする意味が無い
//    🚨 **代わりに、3 つの壊れ方を測って確かめた**（2026-08-16・台の上で）:
//      🔴 読み込みが空（1 ファイル / 0 文字）→ **下の門で exit 1**
//      🔴 60 文字に痩せさせる（**0 ではない**）→ **`missing-tag` で exit 1**
//         （`<LeftSidebar …>` `<MobileNav …>` が見つからない＝**判定が「対象を取れない」と言う**）
//      🔴 範囲が広がる … 🚨 **起きえない**（読むファイルが `LAYOUT_FILE` 固定）
//      🟢 実物 1 ファイル / 13840 文字 → exit 0
//    🚨 **揃えに来ないでください。** 複数ファイルを歩く形へ変えるなら、そのときに要ります。
//
