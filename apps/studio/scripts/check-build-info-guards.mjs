...
for (const c of checks) {
  ...
  if (c.ある) {
    ...
    // 🚨 **✖ の隣に「条件」を置かない**（2026-08-16・自分で読み違えた）。
    //    検査の名前は**満たすべき条件**（「CI が dirty を宣言して**いない**」）なので、
    //    `✖ <条件>` と並ぶと **二重否定**になり、**逆の意味に読める**。
    //    実際、私はこの出力を見て「渡していないのに赤い？」と一瞬読み違えた。
    //    → **破れた条件である**と明示する。すぐ下の「何が起きているか」が観測した事実。
    console.error(`  ✖ 破れた条件: ${c.名前}`);
...
// 🚨 **検査の本数にも下限を置く**（2026-08-16・自分で壊して見つけた）。
//    それまでは **7 件のうち 3 件を消しても exit 0** だった——＝ **痩せても緑のまま**。
//    読めた量の下限（上の `読めた量の下限`）は「ファイルを読めたか」しか見ていない。
//    **読んだあと何件試したか**は別の話で、そこが空いていた。
// 🚨 この下限は【書いただけ】＝ 自動では追随しない。
//    **検査を足したらこの数も上げる**（減らすときは理由をコミット本文に書く）。
const 検査の本数の下限 = 7;
if (checks.length < 検査の本数の下限) {
  console.error(
    `✖ 検査が ${checks.length} 件しかありません（下限 ${検査の本数の下限}）`
