 ...
 */
// 🚨 **なぜ「床」だけで、比率も平均も入れていないか**（2026-08-16。
//    書かないと、次の人が「揃っていない」と読んで揃えにきます）。
//    守りは 3 つ在り、**壊れ方が違います**:
//      **比率**（走査 ÷ 候補）… 形が変わる（範囲が広がる / 判定が狭まる）
//      **平均**（文字数 ÷ 本数）… 本数はそのままで痩せる
//      **床**（実測から遠い絶対値）… 丸ごと減る
//    🚨 この検査は **`read()` を 3 回、リテラルのパスで呼ぶだけ**です（glob も readdir も無い）。
//      → **候補を列挙していない**ので、**比率の分母が作れません**。
//      → **本数が 3 で固定**なので、**平均は「合計 ÷ 3」＝ 1 本ごとの床と同じ意味**になります。
//      → **範囲が広がることも構造上ありません**（読む先が増えようがない）。
//    ＝ **この構造で意味を持つのは床だけ**。他の 2 つは「入れていない」ではなく「作れない」。
//
