...
// ── 自己検査（囮）。**両方向を置く**（検出できること／検出してはいけないこと）──────
{
  ...
  console.log(`  ${positive && positive.allowed === false ? "✅" : "🚨"} 囮(+): 実コードの ${probe} → ` +
    `${positive ? `検出（${positive.kind}）${positive.allowed ? "🚨 だが許容された" : "・違反として集める"}` : "検出できず"}`);
...
//    （私が作ると「在りえない形」ばかりになる）。期待は design の実測どおり。
{
  ...
  // 🚨 **「見逃している形が、実際に使われているか」は別の問い。** 描画で測るしかない。
  //    design（w4A:p22）が 2026-08-16 に測った結果を、出どころつきで置く。
  //    🚨 **これは「その時点の事実」で、古くなる。** 次に読む人は日付と HEAD を見ること。
  console.log("  📌 描画で測った結果（**静的では見えないので、描画側の実測を借りる**）:");
  console.log("        25 ページ × 2 画面幅 = 50 回 ／ 深さ 0 … 19 / 深さ 1 … 31 / 🚨 **深さ 2 以上 … 0**");
  console.log("        ＝ 上の「見逃す」形は、**この時点では実際に使われていない**");
  console.log("        出どころ: design(w4A:p22) / 2026-08-16 / HEAD 3571373 / sp 390・pc 1440 / lang ja");
  console.log("        🚨 測っていない: **動的ルート 4 種類**（実データが無いか一覧から辿れない）");
  console.log("        🚨 **この行は古くなります。** 面を足す作業のあとは、描画側で測り直してください");
...
// 🚨 **なぜ「比率」を選んだか**（次の人が「揃っていない」と読んで揃えにこないように）:
//    この検査は **候補 133 → 走査 86 と間引く**ので、**比率が動く**。
//    （`check-page-actions` は間引かず全部読むので、あちらは**平均文字数**。**揃えないのが正しい**）
// 🚨 **そして比率だけでは足りない**（design の実測・2026-08-16）:
//    **候補と走査が同じ割合で一緒に減ると、比率は動かない**（8 割消えても 0.64 のまま）。
//    → **床**（実測値から遠い絶対値）で「丸ごと減る」を捕まえる。
//    🚨 床は**実測値の近く（86 に対する 80 など）にしない**。近い床は少し減るだけで鳴って腐る。
const SCAN_RATIO_BASELINE = 0.65; // 2026-08-16 実測（86 / 133）
const SCAN_FLOOR = 25; // 🚨 86 から遠い床。app/ と components/ がここを下回ることは想定しない
if (scannedFiles > 0 && scannedFiles < SCAN_FLOOR) {
  console.error(`\n🚨 走査したのが ${scannedFiles} 本です（床 ${SCAN_FLOOR} 本を下回りました）。`);
  console.error("   **候補ごと減っている＝走査そのものが途中で止まっている**可能性があります");
  console.error("   （比率は候補と一緒に減るので動きません。**床だけが見ています**）。");
  process.exit(1);
}
...
// ── 🚨 **集めた違反を報告して落とす** ────────────────────────────────
// 由来: 2026-08-16。`hits` は 113 行目で作られ 163 行目で push されるだけで、
//   **どこからも読まれていなかった**。＝ **面の入れ子を実際に書いても、この検査は落ちない**。
//   実測でそこへ辿り着いた道:
//     ① 囮を 5 通り置いても違反として出ない → 「囮が届いていない」と読みかけた
//     ② 🚨 **いま許容されている実在の行**から許容規則を外しても、違反として出なかった
//        ＝ **囮の置き方ではなく、検査側**だと分かった
//     ③ `hits` の参照箇所を数えたら **push の 1 箇所だけ**だった
//     ④ probe を入れて測ると **hits: 1 件**（`files-manager.tsx:99 囲む罫線`）＝ **集めてはいた**
//   🚨 **自己検査は「違反として報告」と言っていたが、報告先が無かった**（文言も直した）。
//   ＝ **「緑だったが、守っていなかった」**。`nav-parity` が門に載っていなかったのと同じ形。
// 🚨 いまの木の違反は **0 件**（clean な clone で probe を入れて実測）。
//   だから**この行を足しても、誰のコミットも止まらない**。止まるのは**次に入れ子を書いた人**。
if (hits.length > 0) {
  console.error(`\n🚨 面の入れ子: ${hits.length} 件（面の中に、もう 1 枚面を描いています）`);
  for (const h of hits) console.error(`   ${h.file}:${h.line}  ${h.kind}  ${h.snippet}`);
  console.error(
    "\n   直し方: 内側の面をやめる（罫線・背景・影を外す）か、" +
      "**面が要る理由**を ALLOW へ規則として足す（**決定の出どころを添えること**）。",
  );
  process.exit(1);
}

console.log(`\n面の入れ子: 0 件（走査 ${scannedFiles} ファイル ／ 許容 ${allowed.length} 件）`);
