...
//
// 🚨 **この 40 は「アプリのページ」で較正した値**。**部品カタログには当たらない**（2026-08-15 実測）。
//    Storybook（:3104）の story を測ると 15 文字で、**全 6 本が「描画されていない」と出た**。
//    story は元々文字が少ないだけで、壊れていない（同じ実行で面の測定は成功している）。
//    ＝ **較正した母集団の外へ計器を持ち出すと、正常を異常と呼ぶ。**
//    Storybook を測るときは、この行を**壊れている証拠として読まないこと**
//    （下の違反メッセージに実際の文字数としきい値を併記してあるので、そこで判断する）。
