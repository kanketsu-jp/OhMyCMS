 ...
 *
 * 🚨 **この検査は「名前（形）で探している」。送り方を変えられたら何も見なくなる**（2026-08-16 実測）。
 *    探しているのは `JSON.stringify({…})` **1 形だけ**。同じ日に repo を数えた:
 *      JSON.stringify({  **22**  ← これを探している
 *      new FormData(      **6**  🚨 **別の作り方が実在する**
 *      URLSearchParams(  **12**  🚨 同上
 *    ＝ この画面が `FormData` へ変わったら、**この検査は 0 件のまま緑**になる
 *      （「違反が無い」ではなく「**探している形が無い**」）。
 *    🚨 いま `JSON.stringify({` が消えたら「解析できません」で落ちる作りにはしてあるが、
 *      **落ちたあと FormData 用に書き直すのは、次に触る人の仕事**。
 *
