...
const nextConfig: NextConfig = {
   ...
   * 安全側のヘッダの**既定**。
   *
   * 🚨 **「全応答」ではない。** 当初この行に「全応答に付ける」と書いたが、実測で反例が出た:
   *    - 🟢 付く … 画面(200) / API の JSON(200) / 405 / POST の 400 / 307 / `_next/static` の 404
   *    - 🚨 付かない … **未処理の例外による 500**（storage の実測。**私は再現していない**）
   *    ＝ **例外の応答は既定の外に出る**。ただしそれは「そういう口が在る」のではなく
   *      **500 を返していること自体が欠陥**なので、**欠陥を計測器として当てにしない**。
   *
