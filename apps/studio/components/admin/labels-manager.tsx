                ...
                {editing?.id === label.id ? (
                  <>
                    <Input
                       ...
                       *
                       * 🚨 続きの行を `*` で始めているのは飾りではない（2026-08-16）。
                       * `check-control-height-tokens` の `isComment()` は**行の先頭だけ**を見るので、
                       * `*` の無い続きの行は**コメントだと分からず、この戒めの文自体が違反として数えられた**
                       * （3 件。うち 2 件はこの段落、1 件は上の `h-14`）。
                       * 検査は「コメント行は見ない」と書いてあるが、**囮が `//` しか試していない**ため
                       * ブロックコメントの続きが抜けている。**検査側の直しは持ち主に出した。** */
                      className="max-w-64"
                    />
                    <Button
                      size="sm"
