...
export function FolderGrid({ folders }: { folders: FolderRow[] }) {
   ...
   *
   * **意図的にそうしている**わけではなく、**形が合わないため**です:
   *    `useSubmitOnce` は「同じ鍵の実行を1本にする」もので、
   *    ここは **複数のファイルを順に PATCH する**ので、鍵の取り方を決める必要があります
   *    （フォルダ id か、ファイル id の集合か）。
   *
   * 🚨 **黙って落ちる形ではありません**——try/catch が在り、
   *    **何件動いて何件落ちたか**を出しています。危ないのは**二重送信**だけです。
   * 🚨 **掴んで放す操作の二重発火は、実機で見たことがありません（測っていません）。**
   *    測らずに守りを足すと「効いていない守り」が1つ増えるので、**いまは足していません。**
   */
