  ...
  return (
    <input
      ...
      className={cn(
        ...
        // まず**編集できる側**を濃くした（上の `bg-input` = 229）。
        //
        // 🚨 **そのとき、ここは `bg-muted/40`（251）のままにした。その判断が誤りだった。**
        //   auth（w4A:p28）が算術で指摘:「**読み取り専用 251 ⇄ 面 255 の差は 4 のまま**」
        //   ＝ 🚨 それでは §1-8 の由来（「フィールドの枠がないので、わからない」）を
        //     **満たしていない**。私は AE1 のとき**クラスが付いたことしか確かめず、
        //     面との差を数で見ていなかった**（＝ 見た目の合図が無いまま「入れた」と報告していた）。
        // → `bg-muted`（**245**・α を外す）にした。3 つの差が同時に立つ:
        //     面 255 ⇄ 読み取り専用 245 …… **10**（欄が在ることが分かる・§1-8）
        //     読み取り専用 245 ⇄ 編集できる 229 … **16**（編集できる欄が分かる・§1-12）
        //   🚨 **表示モードでは全部の欄が読み取り専用**になるので、ここが薄すぎると
        //     「欄が在る」の合図が消える。**状態ごとに要求が違う**ことを、両方の数で確かめる。
