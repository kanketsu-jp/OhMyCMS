...
- [不具合報告の画面写真は、ファイル一覧に出さない](./decisions/report-screenshots-are-not-library-files.md) — `decisions/report-screenshots-are-not-library-files`
- [関連は右パネルで開く（ページを離れない）。ただし関連の欄はまだ無い](./decisions/relations-open-in-the-right-panel.md) — `decisions/relations-open-in-the-right-panel`
