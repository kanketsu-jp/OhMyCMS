...
| `lib/files` | ファイル関連サービス |
| `lib/storage` | S3互換ストレージのアダプタ（`driver.ts` `s3.ts` `local.ts` `index.ts`。D-001 の「S3互換アダプタ方式」を実装） |
| `lib/db` | Knex 接続（`knex.ts`）と `knexfile.ts`、`db/migrations`（14本、2026-08-13時点） |
