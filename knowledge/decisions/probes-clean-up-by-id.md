...
description: 共有 DB に使い捨ての利用者を作る道具（dev-login）は在るのに、消す作法がリポジトリに無い。2026-08-16 に 2 人が同じ穴に落ちた（片方は 2 人作って後始末を書いておらず、司令塔が全員に心当たりを聞くまで残った。片方は台本 103 本に DELETE が 0 本＝本人の名乗り）。他人のものは消さない・自分の台が作ったものは自分が id で消す・常設 fixture は対象外、の 3 つを噛み合わせて決める。確認は総数ではなく id の残存で行う。行が増える原因は email に時刻を混ぜていることだけなので、いちばん強い対処は消すことではなく email を固定して作らないこと。消す口は HTTP に無く（後始末のために DELETE /api/users を作らない）、共有 DB の行には履歴が無いため、消し忘れではなく名乗らないことが事故になる。
