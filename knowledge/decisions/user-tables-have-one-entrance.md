...
### 実測（2026-08-16・共有 DB・自分たちの行だけ）

design が「断られること」を、私が「印を消すとどうなるか」を測った。

| 状態 | `body_rich_plain`（②のみ） | `deleted_at`（②と③） |
|---|---|---|
| `readonly = true`（既定） | **400** | **400** |
| 🚨 `readonly = false` にする | 🚨 **200**（**本当に書けた**） | **400**（**効き続ける**） |
| 印を戻す | **400** | — |

🟢 対照 `title` を書く … **200**（＝ 口は生きている。断りは特定の欄だけ）。
🟢 design の実測 … 400 のあと **DB を見て、値が入っていないこと**まで確認
（**400 は「断った」であって「書けていない」ではない**）。

🚨 **＝ ③の存在価値が、これで実測になった。**
**②だけの列は印を消せば書ける。③に在る列は、印に関係なく書けない。**
後始末は **id と文字列**で確かめた（総数では確かめない）。
