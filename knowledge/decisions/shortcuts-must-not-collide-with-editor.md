...
- `save`(`mod+enter`) は `whileTyping: true` で登録されている（`page-action.tsx` の
  ...
  🚨 **2026-08-17 追記: `SHORTCUTS.submit` を登録しているのは `report-thread.tsx` 1 本になった。**
  堀池さんの指示（I3 原文「送信ボタンのショートカット（Command + Enter）は、他の画面とキーが
  被ってしまうため、今回は設定なしにしてください」）で `bug-report-composer.tsx` から外した。
  🚨 **原文の鍵と、実際に付いていた鍵が違った。** 実測（headless・:3102・`/admin/reports` の報告フォーム。
  🚨 `fetch` を差し替えて記録だけ取ったので **DB には 1 行も書いていない**）:
  外す前 … **⌘⇧Enter で POST 1 回 / ⌘Enter で 0 回** ／ 🔴 陰性対照 ⌘⇧B で 0 回。
  外した後 … **両方 0 回** ／ 🟢 対照 送信ボタンを押すと 1 回（フォームは壊れていない）。
  ＝ 堀池さんが見ていたのは ⌘Enter だが、**この画面に付いていたのは ⌘⇧Enter** だった。
  どちらにせよ「設定なしに」なので外した。**`SHORTCUTS` の定義そのものは変えていない。**
...
## 🚨 この決定は 1 つの軸しか見ていない（もう 1 つ在る）

**私たちの判断は「何の中に居るか」だけ**（本文の中／入力の中なら鳴らさない）。
**Directus は「どこに居るか」も見ている**（base2 の実測・`use-shortcut.ts:45-53`）:

```
Directus の門 … ①登録した要素そのもの ②その中に焦点が在る ③どこにも焦点が無い
   🚨 **タグ名（INPUT / TEXTAREA）も contentEditable も見ていない**
私たち …… **何の中か**（`isTyping()`）だけ
＝ 🚨 **私たちは「別の領域に焦点が在るのに、主フォームが保存される」を止められない**
```

## 🚨 ただし、いまは起こせない（**「異常が無い 0」ではない**）

司令塔が headless で測った（2026-08-17）:

```
探したもの … **フォームが 2 つ以上ある画面**（＝「別の領域」を作れる画面）
   general / storage / labels / agents / collections / profile を引いた
   🚨 **2 つ以上在るのは `/admin/profile` の 1 枚だけ**
`/admin/profile` … フォーム 2 つ ／ 🚨 **見える submit ボタン 0** ／ 🚨 **`⌘↵` の表示も無し**
   ＝ **`useShortcut` が `form && primary` で降りている**＝ **保存の鍵がそもそも効かない画面**
```

🚨 **「衝突は起きない」ではなく「そういう画面がまだ無い」。**
**踏める人が 0 人なので、いま直す必要は無い。**

### 🚨 起こせるようになる条件（**揃ったら測り直す**）

```
① 1 つの画面に、保存できるフォームが 2 つ以上ある
② そのうち 1 つが主ボタン（`form` ＋ `role="primary"`）を持つ
＝ 🚨 **右パネルに入力を入れると、この条件が揃う**（**堀池さん判断待ち**）
```

**書いておく理由**: これを書かないと、
**右パネルに入力を入れた人が、この話を最初から発見し直す。**
