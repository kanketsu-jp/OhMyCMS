...
### コレクションの表示名も同じ場所に持つ（設問318 A・2026-08-16）

欄名と同じ理由で、**コレクションの表示名**は `directus_collections.translations` に持つ。
この CMS には**コレクションの表示名を置く場所が 1 つも無く**、ゴミ箱が `note`（画面のラベルは
「メモ」）を見出しに使っていた——**利用者が説明のつもりで書いた文が見出しになる**。

- 読むのは `lib/schema/collection-labels.ts` の `collectionLabel(collection, locale)`。
  🚨 **中身は `fieldLabel` を呼ぶだけ**（規則を写経しない＝**欄とコレクションで名前の出方が割れない**）
- 🚨 **画面側は未着手**。`note` を表示名に使っている箇所（ゴミ箱の `labels.get(collection) ?? collection`）は
  まだ差し替えていない。**API とデータまでが入っている状態**
- 🚨 堀池さんの備考「**directusを見習って。基本全てに翻訳をつけれる**」（設問318）。
  いま翻訳を持つのは**フィールドとコレクションの 2 つだけで、残りは足していない**
  （roles / policies / presets などは**未着手**）
