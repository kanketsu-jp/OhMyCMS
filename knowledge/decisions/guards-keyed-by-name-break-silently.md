---
type: decision
title: 欄の名前で効く守りは、名前を変えた瞬間に黙って外れる
description: 下書きの保存除外のように「欄の name / id を正規表現で見て弾く」守りは、表示文言を直すついでに名前を変えると、何も壊れず何も鳴らないまま外れる。名前に依存する守りを見つけたら、その旨をその場に書き、名前を変えないことを受入で確かめる。初期設定の「パスワード」を「管理者コード」と呼び替える作業で、実際に一歩手前まで行った。
tags: [security, design, forms, i18n]
status: active
generated:
  by: agent
  at: 2026-08-16
verified: []
sources:
  - resource: "repo://apps/studio/components/admin/form-draft.tsx"
  - resource: "repo://apps/studio/components/admin/onboarding-form.tsx"
stale_after: 2027-02-16
x_rag_okf:
  id: decisions/guards-keyed-by-name-break-silently
  authorship: agent
---

# 欄の名前で効く守りは、名前を変えた瞬間に黙って外れる

## 決定

**欄の `name` / `id` を見て効く守りが在るなら、その欄の近くに「名前を変えるな」と書く。**
そして **受入で「実際に保存されていないこと」を確かめる**（「除外されるはず」で終わらせない）。

## 何が起きかけたか（2026-08-16・初期設定の 3 段化）

```
`components/admin/form-draft.tsx`（下書きを localStorage に残す部品・**14 画面で使用**）
  🚨 数え方: `grep -rl 'FormDraft\|form-draft' apps/studio/{app,components}`（自身を除く）
  🚨 2026-08-16 の実測。**この数は増えます**（同日中に 12 → 14 になりました）
  SECRET_FIELD_PATTERN = /password|secret|token|key/i
  → 欄の name / id がこれに当たると **保存しない**（秘密を localStorage に置かない判断）
```

ご指示（298）は **「パスワードではなく『管理者コード』という表現にする」**。
表現を変えるとき、**欄の `id` も `admin-code` にしたくなる**。

```
【測った】/password|secret|token|key/i に当たるか
  new-password  → **当たる**（保存されない）
  admin-code    → 🚨 **当たらない**（保存される）
```

🚨 **規則を曲げていないのに、結果は「秘密を localStorage に置く」に変わる。**
しかも **何も壊れず、何も鳴らない**。

## だからこうする

```
✅ **見える文言と、内部の名前を分ける**
   表示は辞書で「管理者コード」／欄は `id="new-password"` のまま
✅ その欄の**すぐ横に理由を書く**（「id から password を外すと守りが外れる」）
✅ 受入で **localStorage を実際に見る**
```

**実測（2026-08-16・Storybook + headless）:**

```
① 管理者コード欄に値を入れる → ② 次へ → ③ テナント名・サービス名に値を入れる
localStorage:
  "ohmycms:draft:/iframe.html:onboarding-details-form":
    {"project-name":["PROBE-SERVICE-9911"],"tenant-name":["PROBE-TENANT-9911"]}
🚨 管理者コードの値は **入っていない**
🟢 対照 テナント名・サービス名は **入っている**（＝ この確かめ方は「在る」も出せる）
```

## 🚨 これは「案として選ぶ」ものではない

同じ結果になる 3 つの道が在り、**危なさが違う**。

| | やること | 結果 |
|---|---|---|
| A | 表示だけ変える（採用） | 秘密は保存されない |
| B | `SECRET_FIELD_PATTERN` を曲げる | 秘密が保存される。**曲げた自覚が在る** |
| 🚨 C | 欄の名前を変える | 秘密が保存される。**曲げた自覚が無い** |

🚨 **C がいちばん危ない。** 「規則を守っている」つもりのまま、結果は B と同じになる。

## より強い形（未実施）

**名前ではなく、宣言で除外する**（例: その欄に「保存しない」印を付け、部品はそれを見る）。
🚨 **いまは入れていない。** `form-draft.tsx` は **14 画面**で使われており（2026-08-16 実測）、
除外の判定を変えるとその全部が同時に動くため、**別の作業として切り出す**。
🚨 **待ちではなく、私が止めているもの**（誰の回答も要らない。**やる番が来たらやる**）。

## 🚨 同じ形が、同じ日にあと 3 回出た（2026-08-16）

**この決定の一般形は「あとで人が変える値を、識別子にしない」。**
書いた日のうちに、**独立に 3 回**同じ直し方へ辿り着いた。

| どこ | 直す前（**名前・状態**） | 直した後（**数・不変の鍵**） |
|---|---|---|
| 管理者を消せるか（231 A） | 「誰が最初の人か」 | **管理者が何人か** |
| 確認コードを正直に返すか | 「初期設定が未完了か」 | **利用者が 0 人か** |
| local admin を引く | 「`local-admin@localhost`」 | **利用者 id** |

🚨 **3 つ目には続きが在る。** 「id が空なら email で引き直して埋める」という素直な直しは、
**IdP が `local-admin@localhost` を送るだけで乗っ取れる**（`saml/verify.ts` が
IdP の email で既存行を SAML に結びつけるため）。
＝ 🚨 **名前で引く守りは、直し方の中にも復活する。**
実際の直しは `provider='local'` ＋ `external_identifier IS NULL` ＋ 候補 1 件に絞られた。
**実測: SAML に結びつけた行では 500 のまま、`local_admin_user_id` も書かれない。**

🚨 **「前提が崩れないよう見張る」より「前提が要らない形にする」ほうが強い。**
**前提が要らなければ、前提が崩れても壊れない。**

## 🚨 未解決のまま残しているもの（2026-08-17・SAML の台）

`acceptance/saml/probe-adoption.sh` と `probe-email-overwrite.sh` の `trip()` は、
往復が**サーバに届いていない**ことを **`status:` という文字列の有無**で判定している。

```
if [ "$(grep -cE '^   status:' "${log}")" = "0" ]; then  … 🚨 届いていない
```

🚨 **これは名前で守っている。** `roundtrip.py` の出力の書式が変われば、
**何も鳴らないまま、この弁別だけが外れる**（＝ このファイルが言っている形そのもの）。

**なぜ直さずに残すか**:
- ここで見分けたいのは「**exit が 0 か**」ではなく「**どこまで届いたか**」。
  停止中の枝では**往復は失敗するのが正しい**（ACS が 403 → `roundtrip.py` は exit 1）ので、
  **exit で止めると正しい失敗まで止まる**。
- より強い形は `nameid-of.sh` と同じ「**応答ファイルが空でないか**」だが、
  この 2 本は **ACS まで叩く**ので、**応答ファイルが常に出るとは限らない**。
- ＝ **名前に依存しない判定を、まだ持てていない。**

🚨 **穴が在ることを知っている人が居なくなるのを防ぐために書いている。**
直すなら「届いたかどうか」を、出力の文字列ではないもの（終了コードの区別・
HTTP の記録・応答ファイル）で表せる形にすること。

## レビュー観点

- [ ] 欄の `name` / `id` を見て効く守りが、他にも在るか（**探し方は名前ではなく、正規表現の側から**）
- [ ] 文言を変える作業で、**内部の名前まで一緒に変えていないか**
- [ ] 「除外されるはず」で終わらせず、**保存先を実際に見たか**（在る側の対照つきで）
