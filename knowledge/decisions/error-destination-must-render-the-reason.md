...
2. 受ける口（`errorKeyFromQuery`）と**出す口**（`<ErrorBanner>`）の**両方**を見る。ただし見えるのは
   「**同じ画面のどこかに在る**」ことだけ（別の error を出していても緑）。その 1 段は人が画面で見る
3. `ErrorBanner` 以外の出し方をしていると落ちる（過検出。人が 1 件見に行く）
4. 索引を見るので、まだ `git add` していない変更は見えない
5. 式を解決できないときは落とす（過検出）

### 🚨 その門に、自分で抜け道を通したら素通りした（同じ日・2 時間後）

pages（w4A:p2F）が出した 1 行を、自分の門に当てた:

> **「必須にした」「検査を足した」で安心しない。抜け道を 1 つ自分で挙げて、それを実際に通してみる。**
> 通ってしまったら、その必須・その検査は**まだ何も守っていない**。

```
抜け道 … `errorKeyFromQuery(` の呼び出しは残し、**`<ErrorBanner>` だけ消す**
   🚨 門は **rc=0（緑）**。＝ **堀池さんが報告した状態そのもの**（URL に鍵は付くが画面は無言）が通った
   ＝ 最初の版は「**受ける口が在るか**」しか見ておらず、**出す口を見ていなかった**
```

→ **両方を見るようにした**（`RECEIVERS` の 2 本）。測り直し:

```
🔴 帯だけ消す ……… rc=1「画面へ出す口（<ErrorBanner>）が無い」
🔴 受ける口だけ消す … rc=1「URL の鍵を受ける口（errorKeyFromQuery）が無い」
   🚨 import 行の `errorKeyFromQuery`（括弧なし）は当たらない ＝ 括弧 1 文字が効いている
🟢 戻して … 17 / 17・rc=0 ／ 作業ツリーの差分 0 行
🟢 締める前に確かめた … 受ける口を持つ 7 枚とも `<ErrorBanner` を持つ（＝ 厳しくしても緑が壊れない）
```

🚨 **「盲点として書いてある」ことは、「守っている」ことではない。**
最初の版は盲点 2 として**正しく宣言していた**が、宣言はその穴を塞がない。
**宣言したら、その穴が本当に通るかを 1 回通してみる。**
