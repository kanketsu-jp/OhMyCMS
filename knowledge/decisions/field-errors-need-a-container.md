...
### 🟢 これは目で見て確かめた（base2・本物の入力・`isTrusted: true`）

**コードからの読みだけでは「画面に何が出るか」は言えない**ので、押してもらった。
台は `/admin/files/new-folder`（フォルダ名の欄が 1 つだけの画面）。

```
① 個数を先に … 入力欄 1 個（name / text / 🚨 required: true）／ submit 1 個「作成 ⌘↵」
🟢 対照（押す前）… 「項目の指定が正しくありません」0 件 ／ 🚨 **`name` という語 0 件**
② 🚨 **半角スペース 1 個**を入れる … `form.checkValidity()` → 🟢 true
   ＝ **空欄では `required` がブラウザ側で止め、サーバまで届かない**
③ 押す … 🟢 `/api/folders` **400**（サーバまで届いた）
        🟢 画面の文言 「**項目の指定が正しくありません。**」1 件
        🚨 **`name` という語 … 0 件**（大文字小文字を無視して全文検索）
🟢 対照 … フォルダ数 1 → 1（**作られていない**）
```

サーバは `lib/files/service.ts:1089` で **「nameは必須です」**と投げている。
🚨 **その `name` は、画面に 1 文字も出ていない。** ＝ この決定の前提が、目で見た証拠になった。

### 🚨 その隣で見つかったこと — 文言が読み上げに届いていない

base2 が同じ台で測った（**この 1 画面だけ。他は見ていない**）:

```
🚨 その文言は `[role="alert"]` にも `[aria-live]` にも入っていない
   【測った】`[role=alert],[aria-live]` の中の文字 … **0 件**（**本文には在るのに**）
```

🚨 **目が見えている人には文が出ているが、読み上げの人には「押したのに何も起きない」に見える。**
**「欄名が無い」より重い可能性がある。**

💭 **入れ物を画面へ出すときに、live region も同じ作業で決めるとよい**（2 度触らずに済む）。
🚨 **どう見せるか・どこを live にするかは design の判断**で、この決定は範囲外。
