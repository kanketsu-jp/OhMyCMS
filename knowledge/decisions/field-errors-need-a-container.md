...
## 🚨 見つけたが直していない — 文字列の欄に何でも入る

司令塔が API へ直接投げて測った（2026-08-17・`zz_probe_actions`）:

```
① title（text）に `{"bad":1}` を投げる … 🚨 **201**（**`{"bad":1}` という文字列として入った**）
🟢 対照 ② flag_bool に文字 … 400 `INVALID_VALUE` ／ ③ id を重複 … 409 `ALREADY_EXISTS`
   （＝ **この台は 400 も 409 も出せる**。①だけが通っている）
```

🚨 **これは「どの欄が悪いか」以前の話**——**そもそも悪いと判定していない。**
**型の検証が無い**ので、文字列の欄には何でも入る。

🚨 **①と②は同じ 1 つの直し。** 22P02（②）で欄を言えないのは、
**PostgreSQL が列名を返さないから**（上の表）。言えるようにするには
**投入の前にアプリが値と列の型を突き合わせる**しかなく、それは①を直すことと同じ。
＝ **①を「やらない」と決める限り、②は欄を言えないまま**である。

🚨 **いまは直していない**（利用者が JSON を直接投げる経路は限られる、という司令塔の判断）。
**次に数える人が「型の検証は在る」と読まないよう、ここに残す。**
