...
export function renderTable(results, meta) {
  ...
  lines.push("");
  lines.push(
    `━━ OhMyCMS v0.9 受入ハーネス ━━  ${meta.startedAt}  リポジトリ=${meta.head}  対象=${meta.baseUrl ?? "-"}` +
      // 🚨 対象は焼き込みで古いことがある。リポジトリの HEAD を対象の版として書かない
      `  対象の版=${meta.targetCommit ?? "不明"}` +
  ...
  return JSON.stringify(
    {
      ...
      targetCommit: meta.targetCommit ?? null,
      achieved,
