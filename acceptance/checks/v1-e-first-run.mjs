 ...
 *
 *   ❌【書いただけ】**画面（DOM）は見ていない。** 見ているのは **HTTP の応答と DB の値**だけ。
 *      ボタンの文言・段の数・幅・読み上げ名が壊れても、ここは緑のまま
 *   ❌【書いただけ】**ロゴのアップロードは通していない**（成功経路は 2026-08-15 に手で1回通しただけ）
 *   ❌【書いただけ】**完了画面のリンク先を押していない**（同上。押して 200 を見たのは手作業の1回だけ）
 *   ❌【書いただけ】**400 のときの文言**（`failed_input` / `failed_conflict`）が出るかは見ていない
 *   ❌【書いただけ】**開発ビルドでしか測っていない**。本番ビルド（Docker）では一度も走っていない
 *   ❌【書いただけ】**同時に2本走らせられない**（:3110 / :5437 / コンテナ名が固定）。
 *      並列が要るなら引数化が必要
 *
 * ＝ **緑は「初期設定を API として終えられる」までを言います。**
 * **「初回体験が壊れていない」までは言いません。**
 *
 * 🚨 **上の 6 項目は「書いただけ」です。鳴りません。**
 * （2026-08-16・polish が段差を見つけ、司令塔が「書いただけと鳴るは別。区別して書け」と配った）
 *
 * **なぜ鳴る形にできないか**（作れない理由を書くところまでが規律。手抜きと区別が付かないため）:
 * ```
 * 検査の側（`check-onboarding-contract.mjs`）は鳴らせた。
 *   理由: 🚨 **「見ていないはずの形」を入力として作り、判定へ通せる**。
 *         誰かが規則を広げると **その形が拾えるようになる**＝ **変化が観測できる**。
 *
 * ここ（受入ハーネス）は鳴らせていない。
 *   理由: 🚨 上の 6 つは **「この検査がやっていない操作」** であって、**入力ではない**。
 *         「DOM を見ていない」を鳴らすには、**この検査が DOM を見るようになったこと**を
 *         検知する必要があり、それは **自分のソースを自分で読む**形になる。
 *         🚨 実物と同じ入口を通らない囮になるので、**「囮が通った」が「本物も通る」の
 *         証拠にならない**（2026-08-16 の規律）。
 * ```
 * **例外が 1 つだけ在ります（まだ入れていません）:**
 * ```
 * 「ロゴのアップロードは通していない」は、送っている本文が `project_logo: ""` かどうかで
 * 機械的に分かる。**誰かが実物を送るようにしたら、この行は古くなる。**
 * 🚨 ただし **同じファイルの中のリテラルを自分で読む**形になるので、
 *    上に書いた「自分のソースを読む囮」と同じ弱さを持つ。**入れるかは未決。**
 * ```
 *
