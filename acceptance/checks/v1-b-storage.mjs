...
export async function check(context) {
  ...
  try {
    ...
    //        🚨 **`/:path*` だから全経路に届く、とは書かない**（**設定は結果ではない**）。
    //          【storage が測った・design は再現していない】**実体の無い asset は 500 になり、
    //          その応答には nosniff が 0 行**（＝ **届かない応答が実在する**。saml 経由・2026-08-17）。
    ...
    //   🚨 **②を外して測る逃げ道は無い。** ただし理由は「全経路に届くから」ではない:
    //     **②が届かない口は実在する（500 の応答）。だがそれは未処理の例外であって、それ自体が欠陥**。
    //     ＝ **欠陥を測定点にはできない**（そこで①を測っても、「①が生きている」ではなく
    //     「**500 になった**」を測ることになる）。＝ **結論（逃げ道が無い）は変わらない**。
