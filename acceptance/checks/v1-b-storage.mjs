...
export async function check(context) {
  ...
  try {
    ...
    //     🚨 **その 404 は「何も無いから」ではない**——`5b35214`（storage）が
    //       **実体が無いときを `404 FILE_NOT_STORED` にした**結果（`lib/files/service.ts:497,896`）。
    //       **それ以前は未処理の例外で 500 になっていた**。
    //     ＝ **「異常が無い 0」ではなく「測れていない」**。
    //
    ...
    //     🚨 **3 版のときの「500 は作れなかった」も、言い方が足りなかった**:
    //       作れなかったのは **`5b35214` で直っていたから**であって、
    //       **「500 が起きない作りだった」からではない**。
    //       ＝ **直った後の木で測って、直る前について語らないこと**（今日 3 度目の形）。
    assertions.push(
