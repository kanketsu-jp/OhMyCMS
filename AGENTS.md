...
## 手を動かす作業は oc（OpenCode ワーカー）へ振る

> 由来: 2026-08-17 堀池指示「**全てのペインが OpenCode を MCP およびツールのように使って、
> Claude は考え、計画、判断するだけで、OpenCode が作業をする**」
> 「**安定して本気で OC を全ペインが自発的に使うようにする**」
>
> 🚨 ここに書いてある理由: **Skill は自分で reload できない**（スラッシュコマンドはエージェントが
> 打てない）。便で伝えても会話は消える。**毎セッション必ず読まれるのは AGENTS.md だけ。**

**このリポジトリで手を動かす作業（実装・書き換え・横断調査）が発生したら、既定で oc に振る。**
Claude がやるのは 4 つだけ: **計画・分解・判断・検証**。

```bash
oc 1 --dir /Users/horiikekazuma/Develop/Projects/kk2/cms "<仕様>"   # 読む量が多い（調査・横断）
oc 2 --dir /Users/horiikekazuma/Develop/Projects/kk2/cms "<仕様>"   # 書く量が多い（実装・生成）※既定
oc 3 --dir /Users/horiikekazuma/Develop/Projects/kk2/cms "<仕様>"   # 短い作業・別系統の意見
oc fan tasks.jsonl -j 8 -o .temp/oc/out --notify <司令塔の pane_id>  # 並列（推奨上限 16）
```

MCP でも呼べる: `oc_run` / `oc_fan` / `oc_ls` / `oc_doctor` / `oc_serve`
詳しい規律: Skill `opencode-delegation` ／ スラッシュコマンド `/oc`

### 🚨 モデルの選び方で成否が変わる（2026-08-17 実測）

```
同じ仕様（あるスクリプトの実装を読んで使い方を書く）
  oc2 … 2 回とも timeout（900 秒 / 600 秒）成果物 0
  oc1 … 🟢 1 回で成功・受入基準を全部満たす
```
**読む量が多いなら oc1。** 迷ったら oc2。oc3 は context 262k なので大量に読ませない。

### 🚨 必ず守ること

1. **`rc=0` は「走り切った」であって「正しい」ではない。** 成果物を `git diff` / `grep` で読んで判定する
   （実測: 7 本振って rc=0 が 5 本、成果物での成功は 4 本）
2. **並列に振る前に、排他・依存・境界を決める。** 同じファイルを 2 本に触らせない。**衝突は振った側の責任**
3. **ワーカーは push / PR / CI / 外部送信をしない**（堀池の明示許可が要る操作）
4. 仕様には **対象 / やること / やらないこと / 受入基準 / 前提** を書く

### 🚨 実物を見ずに受入基準を書かない（このリポジトリでの実例・2026-08-17）

司令塔が「ListEmpty の**同じ要素の中か直後の兄弟**に押せるものが在るか」という基準で検査を作らせた。
成果物は基準を全部満たしたが、**偽の違反を出した**——実際のコードは
「**近くの別の `div`**」に置く形が普通だったため。
＝ 🚨 **基準そのものが間違っていた。** 母集合から外れる書き方を 1 つも挙げずに書いたのが原因。

### 🚨 間欠故障がある（あなたの仕様のせいとは限らない）

同じ文面・同じモデル・同じリポジトリで **32 秒 patch=0** と **2744 秒 patch=1** が出ている。
条件では説明できないことが確定済み（2026-08-17 に 6 つの仮説が全部反証された）。

**1 回の失敗で「使えない」と決めない。同じ仕様をもう 1 回振る**——それ自体が n になり、
`oc-metrics.py` が「同じプロンプトで結果が割れた組」として自動で拾う。

### 実績を見る

```bash
python3 ~/.config/kanketsu/oc/bin/oc-metrics.py --dir /Users/horiikekazuma/Develop/Projects/kk2/cms --since all
```
