 ...
 *
 * ■ 🚨 **キーは、いずれ固定でなくなります**（堀池さん 2026-08-16 の備考）
 *   原文: 「ショートカットは必要そうなものを、なるべくたくさん用意するが、
 *          **初期は未設定（ショートカットが空）でいい**。もちろん重複はできない。」
 *   ＝ 将来は **利用者が自分で割り当てる**・**既定は空**・**重複は弾く**。
 *   🚨 **いまここに入っている `key` は、実装が固定で持っている値**です
 *      （2026-08-16 時点の `SHORTCUTS`）。**恒久の割り当てではありません。**
 *   → 読む側は「**この操作にこのキーが必ず割り当たっている**」と決めつけないこと。
 *   🚨【書いただけ】**この注記は、割り当ての機能が入っても鳴りません。**
 *      その機能を作る人が、ここも一緒に直してください。
 *
