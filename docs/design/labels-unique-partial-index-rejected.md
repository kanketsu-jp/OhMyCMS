...
## で、どうすると決めたのか（結論は knowledge 側に在る）

🚨 **この文書は「部分索引を却下した調査」であって、決定そのものではない。**
決定は [soft-deleted-names-stay-taken](../../knowledge/decisions/soft-deleted-names-stay-taken.md)
——**ゴミ箱に在るものの名前は空けない（部分索引を入れない）**。理由は「戻せる」を保証できるのが
その形だけだから（名前を空けると、同名の新しい行を作れてしまい、あとで戻すときに衝突する）。

その決定側には、この文書に無いものが在る:

- 名前がぶつかったときの**言い分け**（`LABEL_EXISTS` と `LABEL_EXISTS_TRASHED` を分ける理由と文言）
- **前提**（名前を空ける唯一の手段は完全削除なので、完全削除と復元が動くこと。
  2026-08-17 に両方 400 で塞がっていた期間が在り、`1cdf06d` で戻った）
- **割り当てには印を立てない**こと（`ohmycms_label_assignments` の `deleted_at` は
  列だけ在って意図して使っていない。**実装漏れではない**）

🚨 ここに辿り道が無かったので足した（`AGENTS.md §8`「調べた結果なら `docs/`、
だからこうするなら `knowledge/`」——**素材から結論へ辿れて初めて、その分け方が働く**）。
